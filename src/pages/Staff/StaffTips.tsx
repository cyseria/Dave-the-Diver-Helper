...
function StaffAvatarChip({ staffId, skills }: { staffId: string; skills?: string[] }) {
  const navigate = useNavigate();
  const s = staffData.find((x) => x.id === staffId);
  if (!s) return null;
  const avatarUrl = getStaffImageUrl(s.name);
  const tooltip = skills?.length ? `${s.name}\n技能：${skills.join(" · ")}` : s.name;

  return (
    <button
      type="button"
      className={styles.staffAvatarChip}
      onClick={() => navigate(`/staff/${s.id}`)}
      title={tooltip}
    >
      <div className={styles.staffAvatar}>
        {avatarUrl ? (
          <img src={avatarUrl} alt={s.name} className={styles.staffAvatarImg} draggable={false} />
        ) : (
          <span className={styles.staffAvatarFallback}>{s.emoji}</span>
        )}
      </div>
      <span className={styles.staffAvatarName}>{s.name}</span>
    </button>
  );
}
            ...
            {LINEUP_PLANS.map((plan) => (
              <div key={plan.id} className={styles.lineupCard}>
                <div className={styles.lineupHeader}>
                  ...
                  <div className={styles.lineupHeaderText}>
                    <span className={styles.lineupName}>{plan.name}</span>
                ...
                <div className={styles.lineupStores}>
                  <div className={styles.lineupStoreBlock}>
                    <span className={styles.lineupStoreLabel}>🏠 主店</span>
                    <div className={styles.lineupSlotList}>
                      {plan.mainStore.map((slot) => (
                        <div key={slot.roleKey} className={styles.lineupSlotRow}>
                          <span className={styles.lineupSlotRole}>{slot.roleLabel}</span>
                          <div className={styles.lineupAvatars}>
                            {slot.members.map((m) => (
                              <StaffAvatarChip key={m.staffId} staffId={m.staffId} skills={m.skills} />
                            ))}
                          </div>
                        </div>
                      ))}
                  ...
                  <div className={styles.lineupStoreBlock}>
                    <span className={styles.lineupStoreLabel}>🏪 分店</span>
                    <div className={styles.lineupSlotList}>
